#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# Qwen3-8B Eagle3 Draft Model Distillation (vLLM + Mooncake TCP) — MI308
#
...
# ROCm/AMD MI308 GPUs; Mooncake TCP transfers hidden states to training GPUs
...
#
# Usage:
#   bash examples/Qwen3_8B_SDDD_MI308_vLLM/run_qwen3_8b.sh
#   bash examples/Qwen3_8B_SDDD_MI308_vLLM/run_qwen3_8b.sh --smoke-test
#   MODEL_PATH=/path/to/model bash examples/Qwen3_8B_SDDD_MI308_vLLM/run_qwen3_8b.sh
# ═══════════════════════════════════════════════════════════════════════════════
set -uo pipefail
...
EXP_NAME="qwen3-8b-eagle3-vllm-mi308"
...
echo "═══════════════════════════════════════════════════════════════"
echo "  Qwen3-8B Eagle3 Draft Model Distillation (vLLM) — MI308"
...
if [ ${EXIT_CODE} -eq 0 ]; then
    echo ">>> Qwen3-8B Eagle3 distillation (vLLM, MI308) completed successfully."
    echo ">>> Logs saved to: ${LOG_FILE}"
else
    echo ">>> Qwen3-8B Eagle3 distillation (vLLM, MI308) failed with exit code ${EXIT_CODE}." >&2
fi
exit ${EXIT_CODE}
